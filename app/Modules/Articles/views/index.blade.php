...
@section('title', 'Управление статьями | KotiksCMS')
...
@section('content')
    ...
    <div class="btn-group" role="group">
        ...
        <a href="{{ route('admin.articles.trash.index') }}" class="btn btn-outline-primary position-relative">
            <i class="bi bi-trash me-1"></i> Корзина
            @if($trashedArticles > 0)
            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                {{ $trashedArticles }}
                <span class="visually-hidden">статьи в корзине</span>
            </span>
            @endif
        </a>
    </div>
    ...
    <div class="page-actions fade-in">
        <div>
            <h1 class="h5 mb-0">Управление статьями</h1>
            <p class="text-muted mb-0" style="font-size: 0.85rem;">
                Всего: {{ $totalArticles }} | 
                Категорий: {{ $totalCategories }} | 
                В корзине: {{ $trashedArticles }}
            </p>
        </div>
    ...
    <div class="card fade-in mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('admin.articles.index') }}" class="row g-2">
                <!-- Поиск -->
                <div class="col-md-4">
                ...
                <table class="table table-hover mb-0">
                    <thead>
                        <tr>
                            ...
                            <th width="30%">
                                <a href="{{ route('admin.articles.index', array_merge(request()->except(['sort_by', 'sort_order']), ['sort_by' => 'title', 'sort_order' => $sortBy == 'title' && $sortOrder == 'asc' ? 'desc' : 'asc'])) }}"
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="table-actions justify-content-end">
                                        <a href="{{ route('admin.articles.edit', $item) }}" class="btn btn-outline-primary btn-sm me-1" title="Редактировать">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button type="button" class="btn btn-outline-danger btn-sm delete-articles-btn"
                                                title="В корзину"
                                                data-articles-id="{{ $item->id }}"
                                                data-articles-title="{{ $item->title }}"
                                                data-delete-url="{{ route('admin.articles.destroy', $item) }}"
                                                data-bs-toggle="modal" data-bs-target="#deleteArticlesModal">
                                ...
                                <td colspan="7" class="text-center py-4">
                                    <div class="text-muted">
                                        ...
                                        @if(request()->has('search'))
                                            <a href="{{ route('admin.articles.index') }}" class="btn btn-primary btn-sm mt-2">Сбросить фильтры</a>
                                        @else
                                            <a href="{{ route('admin.articles.create') }}" class="btn btn-primary btn-sm mt-2">Создать первую статью</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="card-footer border-0 bg-light">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="text-muted small">
                        Показано {{ $articles->firstItem() }} - {{ $articles->lastItem() }} из {{ $articles->total() }}
                    </div>
                    <div>articles
    ...
    <div class="row mt-4">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0"><i class="bi bi-info-circle me-2"></i> О Статьях</h6>
                </div>
                <div class="card-body">
                    <p class="mb-2" style="font-size: 0.85rem;">Управляйте статьями на сайте.</p>
                    <ul class="mb-0" style="font-size: 0.85rem;">
                        <li>Создавайте и редактируйте статьи</li>
                    ...
                    <div class="d-flex align-items-center mb-3">
                        <div class="flex-grow-1">
                            <div class="fw-semibold mb-1" style="font-size: 0.85rem;">
                                <i class="bi bi-link-45deg me-1"></i> API Статей
                            </div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div class="d-flex gap-1">
                                    <a href="{{ url('api/articles/tree') }}" target="_blank" class="btn btn-outline-primary btn-sm copy-btn" title="Открыть API в новой вкладке">
                                        <i class="bi bi-box-arrow-up-right"></i>
                                    </a>
                                    <button class="btn btn-outline-secondary btn-sm copy-btn" data-clipboard-text="{{ url('api/articles/tree') }}" title="Копировать URL API">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div class="d-flex gap-1">
                                    <a href="{{ url('api/articles/categories') }}" target="_blank" class="btn btn-outline-primary btn-sm copy-btn" title="Открыть API в новой вкладке">
                                        <i class="bi bi-box-arrow-up-right"></i>
                                    </a>
                                    <button class="btn btn-outline-secondary btn-sm copy-btn" data-clipboard-text="{{ url('api/articles/categories') }}" title="Копировать URL API">
                                        <i class="bi bi-clipboard"></i>
                                    </button>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="deleteArticlesModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Вы уверены, что хотите переместить статью <strong id="articlesTitleToDelete"></strong> в корзину?</p>
                    <div class="alert alert-info alert-sm mb-0">
                        <i class="bi bi-info-circle me-2"></i> Статья будет доступна для восстановления в течение 30 дней.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Отмена</button>
                    <form id="deleteArticlesForm" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const deleteButtons = document.querySelectorAll('.delete-articles-btn');
        deleteButtons.forEach(btn => {
            btn.addEventListener('click', function() {
                const id = this.dataset.articlesId;
                const title = this.dataset.articlesTitle;
                const url = this.dataset.deleteUrl;
                document.getElementById('articlesTitleToDelete').textContent = title;
                document.getElementById('deleteArticlesForm').action = url;
            });
        });
    });
</script>
