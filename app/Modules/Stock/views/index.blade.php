...
@section('title', 'Управление Акциями | KotiksCMS')
...
@section('content')
    ...
    <div class="btn-group" role="group">
        <a href="{{ route('admin.stock.index') }}" class="btn btn-primary">
            <i class="bi bi-gift"></i> Активные акции
        </a>
        <a href="{{ route('admin.stock.categories.index') }}" class="btn btn-outline-primary">
            <i class="bi bi-tags me-1"></i> Категории
        </a>
        <a href="{{ route('admin.stock.trash.index') }}" class="btn btn-outline-primary position-relative">
            <i class="bi bi-trash me-1"></i> Корзина
            @if($trashedStock > 0)
            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                {{ $trashedStock }}
                <span class="visually-hidden">акций в корзине</span>
            </span>
            @endif
        </a>
    </div>
    ...
    <div class="page-actions fade-in">
        <div>
            <h1 class="h5 mb-0">Управление акциями</h1>
            <p class="text-muted mb-0" style="font-size: 0.85rem;">
                Всего: {{ $totalStock }} | 
                Категорий: {{ $totalCategories }} | 
                В корзине: {{ $trashedStock }}
            </p>
        </div>
        <a href="{{ route('admin.stock.create') }}" class="btn btn-primary">
            <i class="bi bi-plus-circle"></i> Создать акцию
        </a>
    </div>
    ...
    <div class="card fade-in">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">Список акций</h5>
                <div class="text-muted small">Показано {{ $stock->count() }} из {{ $stock->total() }}</div>
            </div>
        </div>
                ...
                <table class="table table-hover mb-0">
                    <thead>
                        <tr>
                            ...
                            <th width="30%">
                                <a href="{{ route('admin.stock.index', array_merge(request()->except(['sort_by', 'sort_order']), ['sort_by' => 'title', 'sort_order' => $sortBy == 'title' && $sortOrder == 'asc' ? 'desc' : 'asc'])) }}"
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="table-actions justify-content-end">
                                        <a href="{{ route('admin.stock.edit', $item) }}" class="btn btn-outline-primary btn-sm me-1" title="Редактировать">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button type="button" class="btn btn-outline-danger btn-sm delete-stock-btn"
                                                title="В корзину"
                                                data-stock-id="{{ $item->id }}"
                                                data-stock-title="{{ $item->title }}"
                                                data-delete-url="{{ route('admin.stock.destroy', $item) }}"
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-4">
                                    <div class="text-muted">
                                        ...
                                        <p class="mt-2">Акции не найдены</p>
                                        @if(request()->has('search'))
                                            <a href="{{ route('admin.stock.index') }}" class="btn btn-primary btn-sm mt-2">Сбросить фильтры</a>
                                        @else
                                            <a href="{{ route('admin.stock.create') }}" class="btn btn-primary btn-sm mt-2">Создать первую акцию</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="card-footer border-0 bg-light">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="text-muted small">
                        Показано {{ $stock->firstItem() }} - {{ $stock->lastItem() }} из {{ $stock->total() }}
                    </div>
                    <div>stock
                        {{ $stock->links('admin::partials.pagination') }}
                    </div>
                </div>
            </div>
        @endif
    ...
    <div class="row mt-4">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0"><i class="bi bi-info-circle me-2"></i> О Акциях</h6>
                </div>
                <div class="card-body">
                    <p class="mb-2" style="font-size: 0.85rem;">Управляйте акционными записями на сайте.</p>
                    <ul class="mb-0" style="font-size: 0.85rem;">
                        <li>Создавайте и редактируйте акции</li>
                    ...
                    <div class="d-flex align-items-center mb-3">
                        <div class="flex-grow-1">
                            <div class="fw-semibold mb-1" style="font-size: 0.85rem;">
                                <i class="bi bi-link-45deg me-1"></i> API Акций
                            </div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div class="d-flex gap-1">
                                    <a href="{{ url('api/stock/tree') }}" target="_blank" class="btn btn-outline-primary btn-sm copy-btn" title="Открыть API в новой вкладке">
                                        <i class="bi bi-box-arrow-up-right"></i>
                                    </a>
                                    <button class="btn btn-outline-secondary btn-sm copy-btn" data-clipboard-text="{{ url('api/stock/tree') }}" title="Копировать URL API">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div class="d-flex gap-1">
                                    <a href="{{ url('api/stock/categories') }}" target="_blank" class="btn btn-outline-primary btn-sm copy-btn" title="Открыть API в новой вкладке">
                                        <i class="bi bi-box-arrow-up-right"></i>
                                    </a>
                                    <button class="btn btn-outline-secondary btn-sm copy-btn" data-clipboard-text="{{ url('api/stock/categories') }}" title="Копировать URL API">
                                        <i class="bi bi-clipboard"></i>
                                    </button>
                                </div>
                            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Вы уверены, что хотите переместить акцию <strong id="stockTitleToDelete"></strong> в корзину?</p>
                    <div class="alert alert-info alert-sm mb-0">
                        <i class="bi bi-info-circle me-2"></i> Акция будет доступна для восстановления в течение 30 дней.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Отмена</button>
                    <form id="deleteStockForm" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                const title = this.dataset.stockTitle;
                const url = this.dataset.deleteUrl;
                document.getElementById('stockTitleToDelete').textContent = title;
                document.getElementById('deleteStockForm').action = url;
            });
        });
    });
</script>
