...
@section('title', 'Корзина акций | KotiksCMS')
...
@section('content')
    ...
    <div class="d-flex mb-4 fade-in">
        <div class="btn-group" role="group">
            ...
            <a href="{{ route('admin.stock.trash.index') }}" class="btn btn-primary position-relative">
    ...
    <div class="page-actions fade-in">
        <div>
            <h1 class="h5 mb-0">Корзина акций</h1>
            <p class="text-muted mb-0" style="font-size: 0.85rem;">В корзине: {{ $trashedCount }} акций</p>
        </div>
        @if($trashedCount > 0)
    ...
    <div class="card fade-in mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('admin.stock.trash.index') }}" class="row g-2">
                <div class="col-md-8">
                ...
                <div class="col-md-2 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.stock.trash.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-clockwise"></i></a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card fade-in">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">Акции в корзине</h5>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="table-actions justify-content-end">
                                        <form action="{{ route('admin.stock.trash.restore', $item->id) }}" method="POST" class="d-inline">
                                            @csrf
                                        ...
                                        <button type="button" class="btn btn-outline-danger btn-sm force-delete-btn"
                                                ...
                                                data-force-url="{{ route('admin.stock.trash.force', $item->id) }}"
        ...
        @if($stock->hasPages())
            <div class="card-footer border-0 bg-light">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="text-muted small">Показано {{ $stock->firstItem() }} - {{ $stock->lastItem() }} из {{ $stock->total() }}</div>
                    <div>{{ $stock->links('admin::partials.pagination') }}</div>
                </div>
            </div>
        @endif
    </div>
    ...
    <div class="card fade-in">
        <div class="card-body text-center py-5">
            <div class="text-muted">
                ...
                <p class="mb-4">Удаленные акции будут отображаться здесь</p>
                <a href="{{ route('admin.stock.index') }}" class="btn btn-primary">
                    <i class="bi bi-arrow-left me-2"></i> Вернуться к акциям
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Вы уверены, что хотите полностью удалить акцию <strong id="stockTitleToForceDelete"></strong>?</p>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-danger alert-sm mb-0">
                        <i class="bi bi-exclamation-triangle me-2"></i> Это действие удалит <strong>{{ $trashedCount }}</strong> акций безвозвратно.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Отмена</button>
                    <form action="{{ route('admin.stock.trash.empty') }}" method="POST" class="d-inline">
                        @csrf
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        forceDeleteBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                document.getElementById('stockTitleToForceDelete').textContent = title;
