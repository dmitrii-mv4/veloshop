...
/**
 * Миграция для создания таблицы предложений товаров (catalog_offers)
 *
...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('catalog_offers', function (Blueprint $table) {
            ...
            $table->foreignIdFor(Product::class, 'product_id')->comment('Ссылка на товар');
        ...
        if (DB::connection()->getDriverName() === 'pgsql') {
            DB::statement('CREATE INDEX catalog_offers_search_idx ON catalog_offers USING GIN (to_tsvector(\'russian\', COALESCE(name, \'\') || \' \' || COALESCE(articul_supplier, \'\')))');
        }
    }
