...
/**
 * Request для редактирования раздела каталога
 * Валидация полей при обновлении существующего раздела
 */
class SectionEditRequest extends FormRequest
{
    ...
    {
        Log::info('Проверка авторизации для редактирования раздела каталога', [
            'user_id' => auth()->id(),
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'slug' => 'required|string|max:255|regex:/^[a-z0-9-]+$/|unique:catalog_sections,slug,' . $sectionId,
            'description' => 'nullable|string|max:2000',
            'parent_id' => [
                'nullable',
                'integer',
                'exists:catalog_sections,id',
                function ($attribute, $value, $fail) use ($sectionId) {
                    if ($value == $sectionId) {
                        $fail('Раздел не может быть родителем для самого себя');
                    }
                }
            ],
    ...
    protected function prepareForValidation(): void
    {
        ...
        Log::debug('Подготовка данных для валидации редактирования раздела', [
            'data' => $this->all(),
            'section_id' => $this->route('section'),
            'user_id' => auth()->id()
        ]);
